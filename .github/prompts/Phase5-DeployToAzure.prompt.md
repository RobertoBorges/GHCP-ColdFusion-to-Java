...
Deploy the migrated Java 21 (Spring Boot) application to Azure using Azure Developer CLI (azd) for streamlined deployment.
...
- Your application code is ready for deployment (built with `mvn package` or `gradle build`)
...
- Set up application settings (e.g., `SPRING_PROFILES_ACTIVE`, `WEBSITES_JAVA_VERSION`)
- Configure Spring Boot datasource properties for database connections
- Configure continuous deployment if needed
- Set up custom domains and SSL certificates if applicable
- Configure scaling rules
- Set up monitoring with Application Insights Java agent

### For Azure Kubernetes Service (AKS) Deployments:
- Ensure container images are built (e.g., `mvn package` + Docker build) and pushed to a container registry
...
- Configure health probes to use Spring Boot Actuator (`/actuator/health`)
...
- Set up monitoring with Application Insights Java agent
- Configure environment variables (`SPRING_PROFILES_ACTIVE`, datasource properties) and secrets
- Configure health probes to use Spring Boot Actuator (`/actuator/health`)
...
3. **Verify Deployment**
   ...
   - Verify monitoring is working properly with Application Insights Java agent
   - Check that authentication with Entra ID (via Spring Security + MSAL4J) is working correctly
